...
class TSNDataSet(data.Dataset):
    ...
    def _parse_list(self):
        ...
        lines = [x.strip() for x in open(self.list_file)]
        tmp = []
            ...
            for i in range(len(parts)-1, -1, -1):
                try:
                    ...
                    if found_numbers == 0:
                        ...
                        path_parts.insert(0, parts[i])
                except ValueError:
                    # 不是数字，作为路径的一部分
                    path_parts.insert(0, parts[i])
            
            # 如果成功找到标签和帧数
            if label is not None and num_frames is not None:
                path = '/'.join(path_parts) if path_parts else ''
                tmp.append([path, str(num_frames), str(label)])
            else:
                # 兼容旧格式：直接按3部分解析
                if len(parts) >= 3:
                    # 尝试将后两部分转为数字
                    try:
                        ...
                        print(f"Warning: Cannot parse line (old format): {line}")
                else:
